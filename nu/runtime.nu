...
def shell-export-meta-path [repo_root: path]: nothing -> path {
  ($repo_root | path join ".devenv" | path join "polyrepo-shell-export.meta")
}

def shell-export-file-stat-line [label: string path_value: path]: nothing -> string {
...
def shell-export-local-input-repos [repo_root: path]: nothing -> list<record> {
  let local_yaml_path = ($repo_root | path join "devenv.local.yaml")
  if not ($local_yaml_path | path exists) {
    return []
  }

  let local_doc = parse-yaml-mapping $local_yaml_path (open --raw $local_yaml_path)
...
def shell-export-fingerprint [repo_root: path]: nothing -> string {
  mut lines = [
    "version\t1"
    $"repo_root\t($repo_root)"
  ]
...
def read-shell-export-meta [repo_root: path]: nothing -> record {
...
def write-shell-export-meta [repo_root: path shell_export_path: path fingerprint: string]: nothing -> nothing {
  let meta_path = shell-export-meta-path $repo_root
  let meta_text = [
